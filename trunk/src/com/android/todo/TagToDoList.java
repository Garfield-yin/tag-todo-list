...
public class TagToDoList extends Activity {
	...
	public void onCreate(Bundle icicle) {
		...
		mStatButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				...
				String s = sDbHelper.countUncheckedEntries(mTagsArrayAdapter
						.getItem(mTagSpinner.getSelectedItemPosition())
						.toString())
						+ "";
				tvCount1.setText(s);
		...
		mAddEntryButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View view) {
				...
				if (USAGE_STATS) {
					...
					sTracker.trackEvent(Analytics.PRESS,
							Analytics.ADD_TASK_BUTTON, Analytics.INTERFACE,
		...
		final OnCheckedChangeListener ccl = CHOICE_MODE ? new OnCheckedChangeListener() {
			public void onCheckedChanged(CompoundButton cb, boolean isChecked) {
				try {
					...
					sDbHelper.setSuperTask(mContextEntry, cb.getText()
							.toString());
					selectTag(mTagSpinner.getSelectedItemPosition());
					CHOICE_MODE = false;
				...
				: new OnCheckedChangeListener() {
					public void onCheckedChanged(CompoundButton cb,
							boolean isChecked) {
						sDbHelper.updateEntry(cb.getText().toString(),
								isChecked);
						selectTag(mTagSpinner.getSelectedItemPosition());
					}
				};
			...
			do {
				...
				if (collapseLayout != null) {
					if (sDbHelper.getFlag(taskName, ToDoDB.KEY_SUBTASKS) > 0) {
						...
						ib
								.setImageResource(collapsed ? android.R.drawable.ic_menu_add
										: android.R.drawable.ic_menu_close_clear_cancel);
						ib.setOnClickListener(new OnClickListener() {
							public void onClick(View v) {
								sDbHelper.setFlag(taskName,
										ToDoDB.KEY_COLLAPSED, (Boolean) ib
												.getTag() ? 0 : 1);
								selectTag(mTagSpinner.getSelectedItemPosition());
							}
						});
	...
	private final boolean applyMenuChoice(MenuItem item) {
		...
		case TAG_UNINDENT_ID:
			final Cursor c = sDbHelper
					.getTasks(mTagsArrayAdapter.getItem(
			...
			if (c.getCount() > 0) {
				...
				c.moveToFirst();
				do {
					sDbHelper.mDb.update(ToDoDB.DB_ENTRY_TABLE, args,
							ToDoDB.KEY_NAME + " = '" + c.getString(name) + "'",
							null);
				} while (c.moveToNext());
			}
			selectTag(mTagSpinner.getSelectedItemPosition());
	...
	/**
	 * Triggers the activity which asks for the tag's name, and when the
	...
	private void removeTag() {
		if (mTagSpinner.getCount() == 1) {
			Utils.showDialog(-1, R.string.impossible_tag_deletion,
					TagToDoList.this);
			return;
		}
	...
	protected void onDestroy() {
		...
		if (USAGE_STATS) {
			...
			if (month != sSettings
					.getInt(Analytics.LAST_SYNCHRONIZED_MONTH, -1)) {
				sTracker.dispatch();
				sSettings.edit().putInt(Analytics.LAST_SYNCHRONIZED_MONTH,
						month).commit();
			}
			sTracker.stop();
		}
	...
	private final void populateFields() {
		...
		if (SYNC_GCAL = sSettings.getBoolean(ConfigScreen.GOOGLE_CALENDAR,
				false)) {
			GoogleCalendar.setLogin(sSettings.getString(
					ConfigScreen.GOOGLE_USERNAME, ""), sSettings.getString(
					ConfigScreen.GOOGLE_PASSWORD, ""));
		}
		...
		if (sTracker == null
				&& (USAGE_STATS = sSettings.getBoolean(
						ConfigScreen.USAGE_STATS, false))) {
	...
	public void onCreateContextMenu(ContextMenu menu, View v,
		...
		submenu.add(0, ENTRY_MOVE_UNDER_TASK_ID, 0,
				R.string.entry_move_under_task);
	...
	/**
	 * The following function is used by the context menu, as well as the
	 * keyboard shortcuts. Needs mContextEntry to be set before being called.
	 * mContextEntry is a string representation of the selected task.
	 * 
	 * @param selectedItem
			...
			mTagSpinner
					.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
						public void onItemSelected(AdapterView<?> av, View v,
								int i, long arg3) {
							sDbHelper.updateEntryParent(mContextEntry,
									mTagsArrayAdapter.getItem(i).toString(), 0);
	...
	/**
	 * This intercepts a keyboard action. The desired shortcut system is
	 * described in the keyboard_help_text string. This system will also be
	 * applied in implementations of this method in other classes(Activities) as
	 * well, where not conflicting with input methods.
	 */
	public boolean dispatchKeyEvent(KeyEvent msg) {
		...
		case (KeyEvent.KEYCODE_P):
			if (msg.isAltPressed()) {
				mTagSpinner
						.setSelection(Utils.iterate(mTagSpinner
								.getSelectedItemPosition(), mTagSpinner
								.getCount(), -1));
				if (BLIND_MODE) {
					sTts.speak(mTagsArrayAdapter.getItem(
							mTagSpinner.getSelectedItemPosition()).toString());
		...
		case (KeyEvent.KEYCODE_D):
			if (msg.isAltPressed()) {
				...
				if (mActiveEntry > -1) {
					mContextEntry = ((CheckBox) (mEntryLayout
							.getChildAt(mActiveEntry))).getText().toString();
		...
		case (KeyEvent.KEYCODE_V):
			if (BLIND_MODE = !BLIND_MODE) {
				sTts = new TTS(this,
						getString(R.string.visually_challenged_mode_enabled));
			} else {
				Utils.showDialog(R.string.notification,
						R.string.visually_challenged_mode_disabled, this);
				sTts.shutdown();
			}
	 ...
	 * 
	 * @param showDialog
	 *            determines whether the notification will actually be shown
	 *            (used only when the return value is of interest)
	...
	/**
	 * Increases or decreases the to-do list size by 50 tasks. This refers to
	 * the number of checked items you still want to have stored.
	 * 
	 * @param direction
