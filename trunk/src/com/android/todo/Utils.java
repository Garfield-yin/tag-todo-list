...
final class Utils {
	 ...
	 */
	public final static int iterate(int currentValue, int maxValue,
			int increment) {
	...
	/**
	 * This function is used to get an audio file name (with prefix, hash code
	 * and extension) from an original string, for example a task which wants an
	 * audio note. It only works on sdcard?!
	 * 
	 * @param original
	...
	public final static long getTimeMillis(int encodedTime, int encodedDate,
		...
		final int encodedTimeDif = encodedTime - c.get(Calendar.HOUR_OF_DAY)
				* 60 - c.get(Calendar.MINUTE);
		if (encodedDate > 0) {
			final int encodedDateDif = encodedDate - 372 * c.get(Calendar.YEAR)
					- 31 * c.get(Calendar.MONTH) - c.get(Calendar.DAY_OF_MONTH);
			if (encodedDateDif < 0
					|| (encodedDateDif == 0 && encodedTimeDif < -1)) {
				return -1;
			}
			...
			if (dayOfWeek == -1) {
				...
				if (encodedTimeDif > 0) {// in the past
					...
					return c.getTimeInMillis() + 604800000 + encodedTimeDif
							* 60000;
				}
			}
		}
	 ...
	 *            context passed to the UI constructors
	 */
	public final static void showDueTasksNotification(String text,
			final Context c) {

		PendingIntent contentIntent = PendingIntent.getActivity(c, 0,
				new Intent(c, NotificationActivity.class), 0);
