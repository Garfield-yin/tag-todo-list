...
public final class ToDoListDB implements DB {
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE " + DATABASE_ENTRY_TABLE + " ("
					+ KEY_ROWID + " integer primary key autoincrement, "
					+ KEY_NAME + " text not null, " + KEY_STATUS + " integer, "
	...
	public String createEntry(String tagName, String entryName) {
		...
		Cursor c = mDb.query(DATABASE_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_PARENT }, KEY_NAME + " = '" + entryName + "'", null, null,
				null, null);
		if (c.getCount() > 0) {
			c.moveToFirst();
	...
	public void deleteTag(String name) {
		...
		Cursor entries = mDb.query(DATABASE_ENTRY_TABLE, new String[] {
	...
	public Cursor getAllTags() {
		return mDb.query(DATABASE_TAG_TABLE,
	...
	public Cursor getEntries(String tag) {
		...
		return mDb.query(DATABASE_ENTRY_TABLE, new String[] { KEY_ROWID,
				KEY_NAME, KEY_STATUS, KEY_PARENT }, (tag != null ? KEY_PARENT
				+ " = '" + tag + "' " : "1=1 ")
				+ "ORDER BY "
				+ KEY_STATUS
				+ " DESC"
	...
	public Cursor getDueEntries() {
		...
		return mDb.query(DATABASE_ENTRY_TABLE, new String[] { KEY_ROWID,
				KEY_NAME, KEY_STATUS }, KEY_EXTRA_OPTIONS + " = 1 AND "
				+ KEY_STATUS + " = 0 AND (" + KEY_DUE_YEAR + " < " + year
				+ " OR (" + KEY_DUE_YEAR + " = " + year + " AND ("
				+ KEY_DUE_MONTH + " < " + month + " OR (" + KEY_DUE_MONTH
				+ " = " + month + " AND " + KEY_DUE_DATE + " <= "
	...
	public Cursor getAllDueEntries() {
		return mDb.query(DATABASE_ENTRY_TABLE,
				new String[] { KEY_ROWID, KEY_NAME, KEY_STATUS, KEY_DUE_YEAR,
						KEY_DUE_MONTH, KEY_DUE_DATE }, KEY_EXTRA_OPTIONS
						+ " = 1 AND " + KEY_STATUS + " = 0", null, null, null,
				null);
	}
	...
	public boolean updateTag(String tagName, String newName) {
		...
		mDb.update(DATABASE_ENTRY_TABLE, args1, KEY_PARENT + " = '" + tagName
				+ "'", null);
		ContentValues args2 = new ContentValues();
		args2.put(KEY_NAME, newName);
		return mDb.update(DATABASE_TAG_TABLE, args2, KEY_NAME + " = '"
				+ tagName + "'", null) > 0;
	}
		...
		if (checked) {
			...
			if (checkedC.getCount() >= limit) {
				...
				Utils.showDialog(R.string.notification,
						R.string.notification_checked_tasks_limit,
						ToDoListDB.mCtx);
	...
	public boolean updateEntry(String entryName, String newName) {
		...
		return mDb.update(DATABASE_ENTRY_TABLE, args, KEY_NAME + " = '"
				+ entryName + "'", null) > 0;
	}
	...
	public void pushEntryDown(String entryName) {
		Cursor c = mDb.query(true, DATABASE_ENTRY_TABLE, new String[] {
				KEY_NAME, KEY_PARENT, KEY_STATUS }, KEY_NAME + " = '"
				+ entryName + "'", null, null, null, null, null);
		...
		Cursor all = mDb.query(true, DATABASE_ENTRY_TABLE, new String[] {
				KEY_NAME, KEY_PARENT, KEY_STATUS }, KEY_PARENT + " = '"
				+ newParent + "'", null, null, null, null, null);
		all.moveToFirst();
		mDb.delete(DATABASE_ENTRY_TABLE, KEY_PARENT + "='" + newParent + "'",
				null);
		String aux;
