...
public final class ToDoDB extends ADB {
	...
	private class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL("CREATE TABLE " + DB_ENTRY_TABLE + " (" + KEY_ROWID
					+ " integer primary key autoincrement, " + KEY_NAME
					+ " text not null, " + KEY_STATUS + " integer, "
					+ KEY_PARENT + " text not null);");
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			if (oldVersion < 86 && newVersion >= 86) {
				...
				final Cursor c = db.query(DB_ENTRY_TABLE, new String[] {
						KEY_NAME, KEY_WRITTEN_NOTE }, null, null, null, null,
						null);
				if (c.getCount() > 0) {
					c.moveToFirst();
					do {
						...
						if (!"".equals(writtenNote) && writtenNote != null) {
							...
							db.update(DB_ENTRY_TABLE, args, KEY_NAME + " = '"
									+ taskName + "'", null);
						}
			...
			if (oldVersion < 92 && newVersion >= 92) {
				try {
					db.execSQL("ALTER TABLE " + DB_ENTRY_TABLE + " ADD "
							+ KEY_COLLAPSED + " INTEGER DEFAULT 0");
				} catch (Exception e) {
				}
			}
	 ...
	 * opened/created. It's private because ToDoDB is a singleton.
	 * 
	 * @param ctx
	...
	public static final void createBackup() {
		try {
			...
			Utils.copy(new File("/data/data/com.android.todo/databases"),
					new File("/sdcard/Tag-ToDo_data/database_backup"));
		} catch (Exception e) {
	...
	public String createEntry(final String tagName, final String task) {
		...
		final Cursor c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_PARENT }, KEY_NAME + " = '" + task + "'", null, null, null,
				null);
		if (c.getCount() > 0) {
			c.moveToFirst();
	...
	public void deleteAlarm(String entryName) {
		if (isDueTimeSet(entryName)) {
			((AlarmManager) mCtx.getSystemService(Context.ALARM_SERVICE))
					.cancel(PendingIntent.getBroadcast(mCtx, entryName
	...
	public void deleteTag(final String name) {
		...
		final Cursor entries = mDb.query(DB_ENTRY_TABLE, new String[] {
				KEY_NAME, KEY_PARENT }, KEY_PARENT + " = '" + name + "'", null,
				null, null, null);
	...
	public final Cursor getAllTags() {
		return mDb.query(DB_TAG_TABLE, new String[] { KEY_ROWID, KEY_NAME },
				null, null, null, null,
	...
	/**
	 * Returns the existing instance of ToDoDB. If it doesn't exist, it's
	 * created (and opened).
	 * 
	 * @param c
	...
	public static final ToDoDB getInstance(Context c) {
		return sInstance != null ? sInstance : (sInstance = new ToDoDB(c)
				.open());
	}
	 ...
	 * 
	 * @param tag
	 *            for which to get all the tasks. If null, all the tasks from
	 *            all the tags will be returned.
	 * @param depth
	 *            of tasks (depth 0 is a task, >0 is a subtask). If -1, all
	 *            tasks, no matter the depth, will be returned.
	 * @param superTask
	 *            filters to subtasks of a certain supertask. If null, this
	 *            filter won't exist.
	...
	public final Cursor getTasks(final String tag, final int depth,
		...
		return mDb.query(DB_ENTRY_TABLE, new String[] { KEY_ROWID, KEY_NAME,
				KEY_STATUS, KEY_PARENT, KEY_SUBTASKS },
				((tag != null ? KEY_PARENT + " = '" + tag + "' " : "1=1 ")
						+ (depth != -1 ? "AND " + KEY_DEPTH + " = " + depth
								+ " " : "") + (superTask != null ? "AND "
						...
						+ DUEDATE_ORDER_TOKEN
						+ ALPHABET_ORDER_TOKEN, null, null, null, null);
	}
	...
	public final int getFlag(final String task, final String key) {
		final Cursor tasks = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
				key }, KEY_NAME + "='" + task + "'", null, null, null, null);
		tasks.moveToFirst();
	...
	public int getPriority(final String entryName) {
		final Cursor entry = mDb.query(DB_ENTRY_TABLE, new String[] {
				KEY_ROWID, KEY_NAME, KEY_PRIORITY }, KEY_NAME + " = '"
	...
	public Cursor getDueEntries() {
		...
		return mDb.query(DB_ENTRY_TABLE, new String[] { KEY_ROWID, KEY_NAME,
				...
				+ " = 0 AND (" + KEY_DUE_YEAR + " < " + year + " OR ("
				+ KEY_DUE_YEAR + " = " + year + " AND (" + KEY_DUE_MONTH
				+ " < " + month + " OR (" + KEY_DUE_MONTH + " = " + month
				+ " AND " + KEY_DUE_DATE + " <= "
				+ gc.get(GregorianCalendar.DAY_OF_MONTH) + "))))", null, null,
				null, null);
	}

	/**
	 * Return a Cursor over the list of unchecked entries with due dates, from
	...
	public Cursor getAllDueEntries() {
		return mDb.query(DB_ENTRY_TABLE, new String[] { KEY_ROWID, KEY_NAME,
				...
				KEY_EXTRA_OPTIONS + " = 1 AND " + KEY_STATUS + " = 0", null,
				null, null, null);
	}
	...
	public String getWrittenNote(String entryName) {
		final Cursor entry = mDb.query(DB_ENTRY_TABLE, new String[] {
				KEY_ROWID, KEY_NAME, KEY_WRITTEN_NOTE }, KEY_NAME + " = '"
				+ entryName + "'", null, null, null, null);
		// for now, assuming we have a task named like this :)
		entry.moveToFirst();
		String note = entry.getString(entry
	...
	public final int countUncheckedEntries() {
		final Cursor c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_ROWID,
				KEY_NAME, KEY_STATUS, KEY_PARENT }, KEY_STATUS + " = 0", null,
				null, null, null);
	...
	public boolean updateTag(final String tagName, final String newName) {
		...
		mDb.update(DB_ENTRY_TABLE, args1, KEY_PARENT + " = '" + tagName + "'",
				null);
	...
	public void updateEntry(String entryName, boolean checked) {
		...
		if (checked) {
			...
			Cursor checkedC = mDb.query(DB_ENTRY_TABLE, new String[] {
					KEY_NAME, KEY_STATUS }, KEY_STATUS + " = 1", null, null,
					null, null);
			...
			if (isDueTimeSet(entryName)) {
				...
				if (isDueDateSet(entryName)) {// single occurence
					alarmManager.set(AlarmManager.RTC_WAKEUP, Utils
							.getTimeMillis(getDueTime(entryName),
									getDueDate(entryName),
									getDueDayOfWeek(entryName)), pi);
				} else {// daily or weekly
					int dayOfWeek = getDueDayOfWeek(entryName);
					alarmManager.setRepeating(AlarmManager.RTC_WAKEUP,
							Utils.getTimeMillis(getDueTime(entryName), -1,
									dayOfWeek), 86400000L * (dayOfWeek > -1 ? 7
									: 1), pi);
				}
			}
		}
	 ...
	 * @param goDown
	 *            applies the same status to subtasks, if true. If false, it
	 *            goes up, to supertasks. If null, only refers to the present
	 *            task.
	...
	private final void updateEntry(String entryName, boolean checked,
		...
		if (goDown == null) {
			...
			Cursor c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
					KEY_SUPERTASK }, KEY_SUPERTASK + "='" + entryName + "'",
					null, null, null, null);
			if (c.getCount() > 0) {
			...
			Cursor c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
					KEY_SUPERTASK }, KEY_NAME + "='" + entryName + "'", null,
					null, null, null);
			if (c.getCount() > 0) {
				c.moveToFirst();
				if (!checked) {
					updateEntry(c.getString(c.getColumnIndex(KEY_SUPERTASK)),
							false, null);
					updateEntry(c.getString(c.getColumnIndex(KEY_SUPERTASK)),
							false, Boolean.FALSE);
				} else {
					...
					// checked
					Cursor subTasksOfSuperTaskC = mDb
							.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
									KEY_SUPERTASK, KEY_STATUS }, KEY_SUPERTASK
									+ "='"
									+ c.getString(c
											.getColumnIndex(KEY_SUPERTASK))
									+ "'", null, null, null, null);
					if (subTasksOfSuperTaskC.getCount() > 0) {
						final int status = subTasksOfSuperTaskC
						...
						if (allChecked) {
							updateEntry(c.getString(c
									.getColumnIndex(KEY_SUPERTASK)), true, null);
							updateEntry(c.getString(c
									.getColumnIndex(KEY_SUPERTASK)), true,
									Boolean.FALSE);
						}
					}
	...
	public void updateEntryParent(final String task, final String newParent,
		...
		if (subtasks.getCount() > 0) {
			...
			subtasks.moveToFirst();
			do {
				updateEntryParent(subtasks.getString(name), newParent,
						depth + 1);
			} while (subtasks.moveToNext());
		}
		subtasks.close();
	...
	public boolean updateEntry(String entryName, int year, int month, int date) {
		...
		return mDb.update(DB_ENTRY_TABLE, args, KEY_NAME + " = '" + entryName
				+ "'", null) > 0;
	}
	...
	public boolean setDueDate(String entryName, boolean b) {
		final Cursor c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_EXTRA_OPTIONS }, KEY_NAME + " = '" + entryName + "'", null,
				null, null, null);
		c.moveToFirst();
		ContentValues args = new ContentValues();
	...
	public final void setWrittenNote(String task, String note) {
		...
		mDb.update(DB_ENTRY_TABLE, args, KEY_NAME + " = '" + task + "'", null);
		setFlag(task, KEY_NOTE_IS_WRITTEN, !"".equals(note) && note != null ? 1
				: 0);
	}

	/**
	 * Sets whether the functions of this class which return Cursors with
	 * entries should also order them by priority
	 * 
	 * @param order
	 * @param ascending
	...
	/**
	 * Sets whether the functions of this class which return Cursors with
	 * entries should also order them alphabetically
	 * 
	 * @param order
	 * @param ascending
	 *            (if false it will be descending)
	 */
	public final static void setAlphabeticalOrder(boolean order,
			boolean ascending) {
		ALPHABET_ORDER_TOKEN = order ? ", " + KEY_NAME
				+ (ascending ? " DESC" : "") : "";
	}

	/**
	 * Sets whether the functions of this class which return Cursors with
	 * entries should also order them by due date
	 * 
	 * @param order
	 * @param ascending
	 *            (if false it will be descending)
	...
	/**
	 * The given task will be subordinate to the new supertask. The supertask
	 * will also be made false. ASSUMPTION: this function is called when the
	 * task isn't checked
	 * 
	 * @param task
	 * @param superTask
	...
	public void setSuperTask(final String task, final String superTask)
		...
		while ((c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_SUPERTASK }, KEY_NAME + " = '" + curTask + "'", null, null,
		...
		mDb.update(DB_ENTRY_TABLE, args, KEY_NAME + " = '" + superTask + "'",
				null);
		c.close();
	...
	public final void pushEntryDown(final String task) {
		Cursor c = mDb.query(true, DB_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_PARENT, KEY_STATUS }, KEY_NAME + " = '" + task + "'", null,
				null, null, null, null);
		...
		Cursor all = mDb.query(true, DB_ENTRY_TABLE, new String[] { KEY_NAME,
				KEY_PARENT, KEY_STATUS },
				KEY_PARENT + " = '" + newParent + "'", null, null, null, null,
				null);
		all.moveToFirst();
	...
	public Cursor getUncheckedEntries(String tag) {
		return mDb.query(DB_ENTRY_TABLE, new String[] { KEY_NAME, KEY_STATUS,
				KEY_PARENT }, (tag != null ? KEY_PARENT + " = '" + tag
				+ "' AND " : "")
	...
	public final void repair() {
		Cursor c = null;
		try {
			c = mDb.query(DB_ENTRY_TABLE, new String[] { KEY_SUBTASKS }, null,
					null, null, null, null);
		} catch (Exception e) {
			try {
				mDb.execSQL("ALTER TABLE " + DB_ENTRY_TABLE + " ADD "
						+ KEY_SUBTASKS + " INTEGER DEFAULT 0");
			} catch (Exception e2) {
			}
		} finally {
